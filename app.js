...
(function () {
  ...
  function finishInterview() {
    ...
    let effects;
    if (ratio >= 0.75) {
      effects = { stats: { money: 12, happy: 5, independence: 3 }, path: { confident: 1 }, flags: { good_job_offer: true }, history: 'Прошёл(а) собеседование блестяще' };
    } else if (ratio >= 0.4) {
      effects = { stats: { money: 6, happy: 1 }, flags: { got_job: true }, history: 'Получил(а) работу' };
    } else {
      effects = { stats: { happy: -3, money: -2 }, path: { setback: 1 }, history: 'Собеседование прошло неудачно' };
    }
    MOL.applyEffects(state.char, effects);
    autoSave();
    notifyAchievements(SHOP.checkAchievements(state.char));
    ...
    SHOP.purchase(
      ...
      () => {
        ...
        while (state.periodIndex < state.stage.periods.length) {
          const events = MOL.getAvailableEvents(state.stage, state.periodIndex, state.char);
          events.forEach((ev) => {
            if (ev.minigame) return; // мини-игры не резолвим автоматически
