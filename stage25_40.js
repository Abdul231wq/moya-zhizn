    ...
    periods: [
      {
        ...
        events: [
          {
            id: 'pet_farewell',
            title: 'Питомец состарился',
            scene: '🐾',
            text: 'Тот самый питомец из детства теперь совсем старенький — годы берут своё.',
            condition: (c) => c.flags.has_pet === true && !c.flags.pet_passed,
            choices: [
              {
                text: 'Проводишь с ним всё свободное время',
                effects: { stats: { happy: 4, resilience: 2 }, path: { loyal: 1 }, flags: { pet_passed: true }, history: 'Тепло попрощался(ась) со старым питомцем' },
              },
              {
                text: 'Заводишь нового питомца взамен',
                effects: { stats: { happy: 5, money: -3 }, flags: { pet_passed: true, has_pet: true }, history: 'В доме появился новый питомец' },
              },
              {
                text: 'Больше не хочешь заводить животных',
                effects: { stats: { happy: -2 }, flags: { pet_passed: true, has_pet: false }, history: 'Попрощался(ась) с питомцем детства' },
              },
            ],
          },
          {
            ...
            choices: [
              {
                text: 'Расти в найме',
                effects: { moneyScale: true, stats: { money: 18, independence: -1 }, path: { careerist: 1 }, flags: { corporate_path: true } },
              },
              {
                text: 'Свой бизнес / проекты',
                effects: { moneyScale: true, stats: { money: 5, independence: 8, resilience: 5 }, path: { entrepreneur_mindset: 2 }, flags: { business_owner: true }, history: 'Запустил своё дело' },
              },
              {
                text: 'Фриланс / гибкий график',
                effects: { moneyScale: true, stats: { independence: 6, money: 4, happy: 3 }, path: { flexible: 1 }, flags: { freelancer: true } },
              },
            ],
          },
          {
            id: 'profession_milestone',
            title: 'Профессиональная веха',
            scene: '🏅',
            text: 'Годы в выбранной профессии начинают давать плоды — или показывают её цену.',
            condition: (c) => !!c.flags.profession,
            choices: [
              {
                text: 'Выкладываешься, чтобы стать одним из лучших в деле',
                effects: { moneyScale: true, stats: { money: 14, intel: 3, health: -3 }, path: { workaholic: 1, careerist: 1 }, flags: { profession_expert: true }, history: 'Стал(а) признанным специалистом в своей профессии' },
              },
              {
                text: 'Остаёшься на комфортном уровне, без гонки',
                effects: { moneyScale: true, stats: { happy: 4, health: 2 }, path: { balanced: 1 } },
              },
              {
                text: 'Разочаровываешься в выбранном пути',
                effects: { moneyScale: true, stats: { happy: -4, resilience: 3 }, path: { reinvent: 1 }, history: 'Усомнился(ась) в выбранной профессии' },
              },
            ],
          },
          {
          ...
          {
            id: 'friends_adult',
            title: 'Взрослая дружба',
            scene: '🍷',
            text: 'Друзья теперь редкость — у всех свои семьи и графики. Держать связь надо осознанно.',
            choices: [
              {
                text: 'Регулярно находишь время на встречи',
                effects: { stats: { social: 5, happy: 3 }, path: { loyal: 1 } },
              },
              {
                text: 'Общение уходит в переписки раз в полгода',
                effects: { stats: { social: -2 }, path: { busy_adult: 1 } },
              },
            ],
          },
        ],
      ...
      },
      {
        ...
        events: [
          ...
          },
          {
            ...
            scene: '👵',
            text: 'Мама и папа уже не те, что раньше. Появляются звонки о здоровье и просьбы о помощи.',
            choices: [
              {
                text: 'Берёшь на себя заботу и расходы',
                effects: { stats: { money: -6, happy: -1 }, path: { caregiver: 1 }, relationships: { mother: 4, father: 4 } },
              },
              {
                text: 'Помогаешь, но соблюдаешь границы',
                effects: { stats: { resilience: 3 }, path: { balanced: 1 } },
              },
              {
                text: 'Дистанцируешься — своих забот хватает',
                effects: { stats: { independence: 3, happy: -3 }, path: { detached: 1 }, relationships: { mother: -3 } },
              },
            ],
          },
          {
          ...
          },
          {
            ...
            scene: '🍀',
            text: 'Выигрыш в лотерею, неожиданное наследство или премия — то, что случается раз в жизни.',
            condition: (c) => Math.random() < 0.015,
            choices: [
              {
                text: 'Вкладываешь с умом',
                effects: { moneyScale: true, stats: { money: 30, resilience: 2 }, path: { pragmatic: 1 }, history: 'Невероятная удача изменила финансовое положение' },
              },
              {
                text: 'Тратишь на мечту прямо сейчас',
                effects: { stats: { happy: 12, money: 12 }, path: { impulsive: 1 }, history: 'Спустил неожиданную удачу на мечту' },
              },
            ],
          },
          {
            ...
            scene: '✨',
            text: 'Совершенно случайное знакомство — с человеком, который перевернёт следующие несколько лет.',
            condition: (c) => Math.random() < 0.012,
            choices: [
              {
                text: 'Идёшь на риск ради этой возможности',
                effects: { stats: { social: 8, happy: 6, independence: 3 }, path: { bold: 1 }, flags: { life_changing_encounter: true }, history: 'Случайная встреча изменила направление жизни' },
              },
              {
                text: 'Вежливо благодаришь и живёшь дальше как жил',
                effects: { stats: { happy: 2 }, path: { cautious: 1 } },
              },
            ],
          },
        ],
